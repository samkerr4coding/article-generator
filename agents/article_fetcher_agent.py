...
class SourceDataFetcher:
    ...
    def _arxiv_abs_to_html(self, abs_url):
        """
        Convert arXiv 'abs' URL to 'html' URL with version, if possible.
        E.g. 'https://arxiv.org/abs/2508.19562v1' --> 'https://arxiv.org/html/2508.19562v1'
        If version is missing, try to fetch latest version with v1.
        """
        match = re.match(r"https?://arxiv\.org/abs/(\d+\.\d+)(v\d+)?", abs_url)
        if match:
    ...
    def fetch_articles_from_feed(self, rss_url, max_articles=10):
        ...
        Fetch up to max_articles article links from a given RSS feed.
        Returns a list of dicts: {'title', 'url'}
        For arXiv, converts to /html/ URL.
        """
        articles = []
        try:
            feed = feedparser.parse(rss_url)
            for entry in feed.entries[:max_articles]:
                ...
                if url:
                    ...
                    if "arxiv.org" in rss_url:
                        url = self._arxiv_abs_to_html(url)
                    articles.append({'title': title, 'url': url})
        except Exception as ex:
            logging.warning(f"Could not parse RSS feed {rss_url}: {ex}")
        return articles
